...
\begin{lemma}
    ...
    \begin{proof}
        We refer to the proof in Lemma~\ref{lem:gamma-delta-2}. If we assume $\gamma\left(T_1[i].\text{label} \rightarrow T_2[j].\text{label}\right) > \gamma(T_2[j] \rightarrow {\Lambda}) + \gamma(\Lambda \rightarrow {T_2[j]})$,
    \end{proof}
\end{lemma}
